...
def main(branch, script, force):
    ...
    # symbol link doesn't work for KUBE_ROOT, bind mount doesn't for docker mounts
    os.chdir('..')
    check('mkdir', '-p', '/go/src/k8s.io/')
    check('mv', 'kubernetes', '/go/src/k8s.io/')
    check('ln', '-s', '/go/src/k8s.io/kubernetes')
    os.chdir('kubernetes')

    check('git', 'remote', 'add', 'upstream', 'https://github.com/kubernetes/kubernetes.git')
    check('git', 'fetch', 'upstream')
    check('go', 'get', 'github.com/golang/lint/golint')
